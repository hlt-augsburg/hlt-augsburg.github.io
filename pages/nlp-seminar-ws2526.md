---
permalink: /nlp-seminar-ws2526
---
<div class="container">
    <div class="row">
        <div class="col-lg-12 text-center">
            <h2>HLT Seminar</h2>
            <h3>Winter Semester 2025/26</h3>
            Time: tba<br/>
            Please e-mail Hanna Schmueck (firstname dot lastname  @uni-a.de) to reserve a slot. <br/><br/>
            <h4>Current Schedule</h4>
            <table class="table table-striped">
                <thead>
                    <tr>
                        <th>Date</th>
                        <th>Speaker</th>
                        <th>Topic</th>
                        <th>Venue</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>22.10.2025 <b>5.30pm</b></td>
                        <td>Maxim Ionov (University of Zaragova)</td>
                        <td>Linking and Querying Linguistic Data: Addressing Variability in Linguistic Annotation </td>
                        <td>ILKA Talk, D-4056</td>
                    </tr>
                    <tr>
                        <td>29.10.2025, 10am</td>
                        <td>Miriam Elia (Lst. Bauer, PhD Defense)</td>
                        <td>A generic & customizable methodology based on quality gates towards certifiable AI in medicine</td>
                        <td>N 1054</td>
                    </tr>
                    <tr>
                        <td>5.11.2025 (9am - 11.30am)</td>
                        <td>Coling Group</td>
                        <td>Housewarming Breakfast with Conference Recaps</td>
                        <td>BCM</td>
                    </tr>
                     <tr>
                        <td>12.11.2025 <b>5.30pm</b></td>
                        <td>Emilie Pagé-Perron, PhD ( (Archaeological Data Service, Department of Archaeology, University of York)</td>
                        <td> (Archaeological Data Service, Department of Archaeology, University of York) </td>
                        <td>ILKA Talk</td>
                    </tr>
                     <tr>
                        <td>13.11.2025 (Thursday evening!)</td>
                        <td>Marc Schulder (Universität Hamburg)</td>
                        <td>AI Meets Human Data Colloquium: Computational Sign Linguistics: A Sign Language Perspective
                        on Language Technologies and Resources
                        </td>
                        <td>N 2045</td>
                    </tr>
                     <tr>
                        <td>19.11.2025</td>
                        <td>Georg Hofmann</td>
                        <td>Confidence Estimation for Extreme Multi-Label Text Classification using Large Language Models (Master Thesis Colloquium)</td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                    <tr>
                        <td>26.11.2025</td>
                        <td>tba</td>
                        <td>tbd</td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                    <tr>
                        <td>3.12.2025</td>
                        <td>Markus Pauly (TU Dortmund)</td>
                        <td>On LLM Evaluations and Psychometrics (joint with Oberseminar Mathematical Statistics and Artificial Intelligence in Medicine (Prof. Sarah Friedrich-Welz)) </td>
                        <td>L1 3008</td>
                    </tr>
                    <tr>
                        <td>10.12.2025</td>
                        <td>tbd</td>
                        <td>tbd </td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                     <tr>
                        <td>17.12.2025</td> <!--Anne cannot make it-->
                       <td>tbd</td>
                        <td>tbd </td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                    <tr> <!-- in this week: likely visit of Roman Klinger (AI Meets Human Data Colloquium) -->
                        <td>7.1.2026</td>
                        <td></td>
                        <td>New Year's Brunch</td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                    <tr>
                        <td>14.1.2026</td>
                        <td>tbd</td>
                        <td>tbd </td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                    <tr>
                        <td>21.1.2026</td>
                        <td>tbd</td>
                        <td>tbd </td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                    <tr>
                        <td>28.1.2026</td>
                        <td>Stefan Grünewald (Bosch Center for Artificial Intelligence, Stuttgart)</td>
                        <td>Syntactic Dependencies and Beyond: Robust Neural Architectures and Quality-Enhanced Corpora for Structured Prediction in NLP </td>
                        <td>BCM preferred, Zoom possible</td>
                    </tr>
                    <tr>
                        <td>2.2.2026 (5.30pm!)</td>
                        <td>Stephanie Evert (FAU Erlangen)</td>
                        <td>ILKA Talk: Studying linguistic variation with Geometric Multivariate Analysis </td>
                        <td>tba</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
